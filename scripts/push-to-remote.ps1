...
git add .
if ($Force) {
    git commit -m "Initial import of shirasameProject" --allow-empty
    if ($LASTEXITCODE -ne 0) { Write-Host "Commit failed or no changes to commit." }
} else {
...
if ($RemoteUrl) {
    ...
    if ($LASTEXITCODE -ne 0) { Write-Host "Could not add remote." }
