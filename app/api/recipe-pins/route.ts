...
export async function GET(req: Request) {
  try {
    ...
    // If recipeId provided via querystring, filter by it
    let query = supabaseAdmin.from('recipe_pins').select('*')
    if (recipeId) query = query.eq('recipe_id', recipeId)
    const { data, error } = await query
    if (error) {
      console.error('[api/recipe-pins] fetch error', error)
      // If the table doesn't exist in the connected DB (common in fresh/dev setups),
      // return an empty array instead of a 500 so clients' warm-cache doesn't spam logs.
