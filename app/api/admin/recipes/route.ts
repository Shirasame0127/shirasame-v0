...
export async function POST(req: Request) {
  try {
    ...
    // Force ownership to configured owner where possible
    try {
      const ownerId = await getOwnerUserId()
      if (ownerId) recipeRow.user_id = ownerId
    } catch (e) {
      // continue without owner enforcement
...
export async function PATCH(req: Request) {
  try {
    ...
    // Enforce owner if configured
    let query = supabaseAdmin.from('recipes').update({ published, updated_at: new Date().toISOString() }).eq('id', id).select()
    try {
      const ownerId = await getOwnerUserId()
      ...
      // ignore owner resolution errors and proceed without owner constraint
    }
...
export async function DELETE(req: Request) {
  try {
    ...
    // Build delete query, enforce owner when possible
    let query = supabaseAdmin.from('recipes').delete().eq('id', id).select()
    try {
      const ownerId = await getOwnerUserId()
