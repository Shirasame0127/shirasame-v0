...
export async function POST(req: Request) {
  try {
    ...
    // Resolve owner user id and ensure only owner can write (best-effort)
    let ownerUserId: string | null = null
    try {
      ownerUserId = await getOwnerUserId()
    } catch (e) {
      // ignore — still allow write but log
      console.warn('[admin/recipe-pins] owner resolution failed', e)
    }

    // Normalize pins for DB insert
    // Ensure the referenced recipe exists to avoid FK violations
    try {
      ...
      // If the recipes table is missing or another DB error occurs, surface a clear message
    ...
    // Delete existing pins for the recipe and insert new rows
    try {
      await supabaseAdmin.from('recipe_pins').delete().eq('recipe_id', recipeId)
      if (rows.length > 0) {
        const { data, error } = await supabaseAdmin.from('recipe_pins').insert(rows).select()
        if (error) {
