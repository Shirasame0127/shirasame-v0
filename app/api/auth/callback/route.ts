...
async function exchangeCodeForTokens(supabaseUrl: string, anonKey: string, code: string, redirectTo: string) {
  // Supabase推奨: grant_type=authorization_code をクエリに、JSONボディで code/redirect_to、apikeyヘッダは anon key
...
export async function GET(req: Request) {
  try {
    ...
    const SERVICE_ROLE = process.env.SUPABASE_SERVICE_ROLE_KEY || '' // session保存用に保持
    ...
    // Persist auth_sessions (best effort)
    try {
      if (refreshToken) {
