...
- データ: Supabase（DB・認証）、画像は Cloudflare R2 にオリジナルを保管し、Cloudflare Image Resizing（`/cdn-cgi/image`）でオンデマンド変換して配信する方針を推奨します（代表幅: 200/400/800、`format=auto`、`quality=75`）。
...
- ステップ:
 ...
 2) メイン画像をアップロード（R2 へ保存）。保存処理は原画像を R2 に置き、DB に canonical な `publicUrl` や `basePath` を保持します。フロントはまず `publicUrl` を優先し、`basePath` がある場合は Cloudflare Image Resizing による代表幅（200/400/800）のオンデマンド変換 URL を組み立てて利用します。
...
## 運用上の注意
 - 画像: 基本はオンデマンド変換で対応します。フロントは canonical な `publicUrl` を優先し、`basePath` がある場合は Cloudflare Image Resizing の代表幅（200/400/800）で変換 URL を生成して使ってください。事前生成はオプションであり、必要に応じて運用判断で導入してください。
...
## トラブルシューティング（簡易）
- フロントのサムネが表示されない
  - 公開 URL（`images.shirasame.com` などのカスタムドメイン、または R2 の公開サブドメイン）へ直接 HEAD/GET して 200 を確認。
  - 画像が表示されない場合は、まず `publicUrl` に対して HEAD/GET を行い公開アクセスがあるか確認してください。`basePath` ベースの固定ファイル名（`thumb-400.jpg` 等）に依存する運用はしていないため、未生成を前提にしないでください。必要なら管理側で `basePath` が正しく保存されているか、あるいはオンデマンド変換（`/cdn-cgi/image`）で期待する幅の URL を生成できるかを確認してください。
- API 取得が失敗する
