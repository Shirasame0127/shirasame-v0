...
- 管理 CRUD API（タグ・コレクション・製品・レシピ等）は public-worker 側で実装され、Worker がフォワードする
 ...
 - 下書き（draft）対応（2025-12-14 追加）: 商品（products）とレシピ（recipes）の新規作成画面に「下書きとして保存」機能を追加しました。下書き保存では画像を必須とせず、公開時のみ画像必須のバリデーションを行います。管理 UI は下書きと公開の二段階保存をサポートしています。
...
### 静的管理画面向け: `PUBLIC_WORKER_API_BASE` とランタイムオーバーライド

- 方針: 管理ページは静的ホスティング（例: Pages / Vercel / S3 + CDN）で配信しつつ、ブラウザの JavaScript が直接 public-worker の API を呼ぶ運用を許容します。その場合、下記いずれかの方法で public-worker のベース URL をクライアントに渡してください。

- 方法 A (推奨 — HTML に埋め込む): 管理ページの HTML に以下を挿入して runtime に API ベースを上書きします。

```html
<script>
  // 公開用 public-worker の実際の URL
  window.__env__ = { API_BASE: 'https://public-worker.shirasame-official.workers.dev' };
</script>
```

- 方法 B (ビルド時 env): `NEXT_PUBLIC_API_BASE_URL` / `BUILD_API_BASE` をビルド時に `https://public-worker.shirasame-official.workers.dev` に設定して静的資産をビルドする（ただしビルド時に同一オリジンである場合、相対 `/api` を使う既存ロジックと衝突する可能性があるため注意）。

- 注意点:
  - 静的管理画面では HttpOnly Cookie が利用できないため、クライアント側で `Authorization: Bearer <token>` を付与する運用が必要になります。`apiFetch()` は `localStorage` の `sb-access-token` を参照して自動的に Authorization ヘッダを付与するよう実装されています。
  - 直接 public-worker を叩く場合、トークンの保持はクライアント側責任となるため、長期トークンの露出に注意してください。

### `/api/images/complete` の動作（public-worker 直接呼び出しまたは admin-proxy 経由）

 - 実装済み: public-worker 側に `POST /api/images/complete` を実装しています。管理サーバ（admin-site）が互換のために用意していた旧来の `POST /api/images/save` の proxy 実装は段階的に廃止し、管理クライアントから public-worker への直接呼び出し（`PUBLIC_WORKER_API_BASE` をランタイムで指定）を推奨します。転送が必要な場合は admin-proxy が `Authorization` ヘッダ／Cookie を保持して `public-worker` に転送します。
- public-worker の URL（現在）: `https://public-worker.shirasame-official.workers.dev/`。

### テスト手順（cURL 例）

- 直接 public-worker を叩く（デバッグ用）:

```powershell
curl.exe -v -X POST "https://public-worker.shirasame-official.workers.dev/api/images/complete" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <YOUR_TOKEN>" \
  -d '{"key":"images/2025/12/08/example.png","filename":"example.png","metadata":{}}'
```

- admin proxy 経由で転送を試す（admin が proxy をホストしている場合）:
...
期待結果: いずれの方法でも最終的に public-worker の `/api/images/complete` が処理され、そのレスポンスがそのまま返る（ステータスと JSON ボディを確認）。
