...
### `/api/images/save` の動作（admin 側 proxy を使う場合）

- 実装済み: 管理サーバ（admin-site）側に `POST /api/images/save` を配置し、受け取ったペイロードを `PUBLIC_WORKER_API_BASE + '/api/images/complete'` に転送する proxy を実装しました。転送時に `Authorization` ヘッダをそのまま転送します。
...
```powershell
curl.exe -v -X POST "https://admin.shirasame.com/api/images/save" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <YOUR_TOKEN>" \
  -d '{"key":"images/2025/12/08/example.png","filename":"example.png","metadata":{}}'
```
