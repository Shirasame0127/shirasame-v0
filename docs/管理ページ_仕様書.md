...
- 実装済み: public-worker 側に `POST /api/images/complete` を実装しています。管理サーバ（admin-site）が古い互換のために用意していた `POST /api/images/save` の proxy 実装は段階的に廃止し、管理クライアントから public-worker への直接呼び出し（`PUBLIC_WORKER_API_BASE` をランタイムで指定）を推奨します。転送が必要な場合は admin-proxy が `Authorization` ヘッダ／Cookie を保持して `public-worker` に転送します。
