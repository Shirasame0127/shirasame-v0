  ...
  return cacheJson(c, key, async () => {
    try {
      ...
      const transformed = {
        ...
        headerImages: (Array.isArray(user.header_image_keys) ? user.header_image_keys.map((k:any) => buildResizedImageUrl(k, { width: 1600 }, c.env.IMAGES_DOMAIN)).filter(Boolean) : null),
          ...
          if (Array.isArray(urows) && urows.length > 0) {
            ...
            try {
              ...
              const normalized: Record<string, any> = {
                ...
                headerImages: (function(keys:any[]){ try { return (Array.isArray(keys) ? keys : []).map(k=> buildResizedImageUrl(k, { width: 1600 }, c.env.IMAGES_DOMAIN)).filter(Boolean) } catch { return [] } })(mapHeaderKeys(u.header_image_keys || u.headerImageKeys)),
        ...
        if (ures.ok) {
          ...
          if (Array.isArray(urows) && urows.length > 0) {
            ...
            const normalized: Record<string, any> = {
              ...
              headerImages: (function(keys:any[]){ try { return (Array.isArray(keys) ? keys : []).map(k=> buildResizedImageUrl(k, { width: 1600 }, c.env.IMAGES_DOMAIN)).filter(Boolean) } catch { return [] } })(mapHeaderKeys(u.header_image_keys || u.headerImageKeys || u.header_images || u.header_images_keys)),
        ...
        if (res.ok) {
          ...
          if (Array.isArray(j) && j.length > 0) {
            ...
            out.user = {
              ...
              headerImages: (function(keys:any[]){ try { return (Array.isArray(keys) ? keys : []).map(k=> buildResizedImageUrl(k, { width: 1600 }, c.env.IMAGES_DOMAIN)).filter(Boolean) } catch { return [] } })(mapHeaderKeys(u.header_image_keys || u.headerImageKeys || u.header_images)),
...
app.get('/api/admin/users/:id', async (c) => {
  try {
    ...
    // No service role: allow authenticated self-read via anon client
    const ctx = await resolveRequestUserContext(c)
    if (ctx && ctx.trusted && ctx.userId && ctx.userId === id) {
      try {
        const supabase = getSupabase(c.env)
        const { data: userRow } = await supabase.from('users').select('*').eq('id', id).limit(1).maybeSingle()
        if (!userRow) {
          const base = { 'Content-Type': 'application/json; charset=utf-8' }
          const merged = Object.assign({}, computeCorsHeaders(c.req.header('Origin') || null, c.env), base)
          return new Response(JSON.stringify({ error: 'not_found' }), { status: 404, headers: merged })
