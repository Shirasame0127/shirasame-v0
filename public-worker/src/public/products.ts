...
export function registerProducts(app: Hono<any>) {
  app.get('/api/public/products', async (c) => {
    try {
      ...
      return await cacheJson(c, key, async () => ({ status: 500, body: { code: 'server_error', message: '商品一覧取得に失敗しました', details }, headers }))
    }
  })
  ...
  app.get('/api/public/products/:id', async (c) => {
    try {
      ...
      if (!data) {
        ...
        return await cacheJson(c, key, async () => ({ status: 404, body: { code: 'not_found', message: '商品が見つかりません' }, headers }))
      }
      const domainOverride = (c.env as any).R2_PUBLIC_URL || (c.env as any).IMAGES_DOMAIN || null
      ...
      return await cacheJson(c, key, async () => ({ status: 500, body: { code: 'server_error', message: '商品取得に失敗しました', details }, headers }))
    }
  })
}
