const tagsPaths = {
  ...
  '/api/admin/tags/save': {
    post: {
      tags: ['タグ'],
      summary: '複数タグの upsert（保存）',
      description: '配列で渡したタグを挿入または更新します',
      responses: { '200': { description: '保存済み' } },
      security: [{ bearerAuth: [] }]
    }
  },
  ...
  '/api/admin/tags/reorder': {
    post: {
      tags: ['タグ'],
      summary: 'タグの並び替え',
      description: 'sort_order と group を更新します',
      responses: { '200': { description: '更新済み' }, '403': { description: '権限なし／見つからない' } },
      security: [{ bearerAuth: [] }]
    }
  }
}
