...
// Sticky offset of the gallery search bar. Single source of truth: the CSS
// `top` and the stick detection must not drift apart (they used to: 74 vs 64).
const GALLERY_SEARCH_STICKY_TOP = 74
 ...
 *
 * This is the pre-existing home page, moved here verbatim so the A/B/C test has
 * an untouched baseline. Do not refactor it: any change here invalidates the
 * comparison. Variants B and C live alongside it and share `useHomeData`.
 */
export default function HomeA() {
  ...
  // 'auto' follows the viewport; switches to 'manual' the moment the visitor
  // picks a column count, so resize stops overwriting their choice.
  const [columnsMode, setColumnsMode] = useState<'auto' | 'manual'>('auto')

  const setGridColumnsManual = useCallback((n: number) => {
    setColumnsMode('manual')
    setGridColumns(n)
  }, [])
  ...
  // Column count is either derived from the viewport ('auto') or pinned by the
  // visitor via the filter panel ('manual'). Previously the resize listener
  // reset it unconditionally, so an explicit choice was silently discarded on
  // any resize — including the address-bar show/hide on mobile, which fires
  // resize constantly and made the masonry reflow mid-scroll.
  const autoColumnsFor = useCallback((mode: 'normal' | 'gallery') => {
    const isMobileViewport = window.innerWidth < 640
    if (isMobileViewport) return 2
    return mode === 'gallery' ? 7 : 5
  }, [])

  useEffect(() => {
    if (typeof window === 'undefined') return
    if (columnsMode !== 'auto') return

    const updateCols = () => setGridColumns(autoColumnsFor(displayMode))
    updateCols()

    let timer: number | null = null
    const onResize = () => {
      if (timer) window.clearTimeout(timer)
      timer = window.setTimeout(updateCols, 150)
    }
  ...
  }, [displayMode, columnsMode, autoColumnsFor])
  ...
  // Detect when the gallery search bar has reached its sticky offset.
  // This used to be a scroll listener comparing against 64px while the CSS
  // sticks at 74px — a 10px dead zone where the white background flickered on
  // and off. An IntersectionObserver with a matching root margin flips exactly
  // at the stick point and does no per-scroll layout reads.
  // (The duplicate setGridColumns() that also lived here is gone; column count
  // is owned by the effect above.)
  useEffect(() => {
    if (typeof window === 'undefined') return
    if (displayMode !== 'gallery') { setIsGallerySearchSticky(false); return }

    const el = document.getElementById('global-gallery-search')
    if (!el) return

    const obs = new IntersectionObserver(
      ([entry]) => setIsGallerySearchSticky(entry.intersectionRatio < 1),
      { rootMargin: `-${GALLERY_SEARCH_STICKY_TOP + 1}px 0px 0px 0px`, threshold: [1] },
    )
    obs.observe(el)
        ...
        <div className={`max-w-7xl mx-auto px-4 pt-8 ${isAllOverlayOpen ? 'all-items-overlay' : ''}`}>
          <div className="mb-8 flex flex-col items-center">
            ...
            <div className="relative inline-flex items-center rounded-full shadow-sm border-2 border-sky-400 bg-white gallery-toggle" style={{ width: 280, padding: '4px' }}>
              <div
                aria-hidden
                className="absolute top-1 bottom-1 rounded-full bg-sky-300 transition-all duration-300 ease-in-out"
                style={{
                  width: 'calc(50% - 4px)',
                  left: displayMode === 'gallery' ? '50%' : '4px',
                }}
              />
          ...
          {displayMode === 'gallery' ? (
            <section id="global-gallery" className="mb-16">
              <div className="gallery-search-viewport" style={{ position: 'sticky', top: `${GALLERY_SEARCH_STICKY_TOP}px`, boxSizing: 'border-box', display: 'flex', justifyContent: 'center', zIndex: isModalOpen ? 0 : 30 }}>
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-3 gap-3">
                    {(() => {
                      ...
                      if (len === 9) {
                        ...
                        const targetRow = Math.floor(oldestIdx / 3)
                        for (let i = 0; i < len; i++) overlayFlags.push(Math.floor(i / 3) === targetRow)
                      }
      ...
      {isAllOverlayOpen && (
        <section id="all-items-view" className={`mb-16 relative z-60 duration-300 ${isAllOverlayClosing ? 'animate-out fade-out-0 slide-out-to-top' : 'animate-in fade-in slide-in-from-top-2'}`}>
          <div className="max-w-7xl mx-auto px-4 pt-16 pb-10 bg-transparent">
            <button aria-label="閉じる" className="absolute top-4 left-4 text-gray-800 hover:text-gray-900 bg-white rounded-full w-10 h-10 flex items-center justify-center shadow" onClick={() => { setIsAllOverlayClosing(true); setTimeout(() => { setIsAllOverlayOpen(false); setIsAllOverlayClosing(false) }, 300) }}>
