  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto">
      <div ref={pinAreaRef} className="relative w-full">
        <img src={(imageUrl ? getPublicImageUrl(imageUrl) : imageDataUrl) || "/placeholder.svg"} alt={recipeTitle} className="w-full h-auto object-contain rounded-md" onLoad={handleImageLoad} />
        {imageLoaded && pins.map((pin) => {
          ...
          const bgWidthPx = pin.tagBackgroundWidthPercent && pin.tagBackgroundWidthPercent > 0 ? (pin.tagBackgroundWidthPercent / 100) * imageWidthPx : undefined
          const bgHeightPx = pin.tagBackgroundHeightPercent && pin.tagBackgroundHeightPercent > 0 ? (pin.tagBackgroundHeightPercent / 100) * imageWidthPx : undefined
          const bgOffsetXPx = pin.tagBackgroundOffsetXPercent ? (pin.tagBackgroundOffsetXPercent / 100) * imageWidthPx : 0
          const bgOffsetYPx = pin.tagBackgroundOffsetYPercent ? (pin.tagBackgroundOffsetYPercent / 100) * imageWidthPx : 0
          ...
          return (
            <div key={pin.id}>
              <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 10 }}>
                <line x1={`${pin.dotXPercent}%`} y1={`${pin.dotYPercent}%`} x2={`${pin.tagXPercent}%`} y2={`${pin.tagYPercent}%`} stroke={pin.lineColor || "#ffffff"} strokeWidth={lineWidthPx} strokeDasharray={pin.lineType === "dashed" ? `${lineWidthPx * 4} ${lineWidthPx * 2}` : pin.lineType === "dotted" ? `${lineWidthPx} ${lineWidthPx}` : undefined} />
              </svg>

              <div className="absolute cursor-pointer hover:scale-125 transition-transform z-20" style={{ left: `${pin.dotXPercent}%`, top: `${pin.dotYPercent}%`, transform: "translate(-50%, -50%)", width: dotSizePx, height: dotSizePx }} onClick={() => onProductClick(product)}>
                <div className="w-full h-full ring-2 ring-white/30" style={{ backgroundColor: pin.dotColor || "#ffffff", borderRadius: pin.dotShape === "circle" ? "50%" : pin.dotShape === "square" ? "0" : pin.dotShape === "diamond" ? "15%" : "0", clipPath: pin.dotShape === "triangle" ? "polygon(50% 0%, 0% 100%, 100% 100%)" : pin.dotShape === "diamond" ? "polygon(50% 0%, 100% 50%, 50% 100%, 0% 50%)" : undefined }} />
              </div>

              <div className="absolute cursor-pointer hover:scale-105 transition-transform whitespace-nowrap z-30" style={{ left: `${pin.tagXPercent}%`, top: `${pin.tagYPercent}%`, transform: "translate(-50%, -50%)" }} onClick={() => onProductClick(product)}>
                <div style={{ position: "absolute", left: "50%", top: "50%", transform: `translate(calc(-50% + ${bgOffsetXPx}px), calc(-50% + ${bgOffsetYPx}px))`, width: bgWidthPx ? bgWidthPx : "100%", height: bgHeightPx ? bgHeightPx : "100%", backgroundColor: pin.tagBackgroundColor || "#000000", opacity: isFinite(pin.tagBackgroundOpacity) ? pin.tagBackgroundOpacity : 0.8, borderRadius: borderRadiusPx, borderWidth: borderWidthPx, borderStyle: "solid", borderColor: pin.tagBorderColor || "#ffffff", boxShadow: pin.tagShadow || "0 2px 8px rgba(0,0,0,0.2)", zIndex: -1 }} />
                <div style={{ fontSize: fontSizePx, fontFamily: pin.tagFontFamily || "system-ui", fontWeight: pin.tagBold ? "bold" : pin.tagFontWeight || "normal", fontStyle: pin.tagItalic ? "italic" : "normal", textDecoration: pin.tagUnderline ? "underline" : "none", textTransform: pin.tagTextTransform || "none", color: pin.tagTextColor || "#ffffff", textShadow: textShadow, WebkitTextStroke: strokeWidthPx > 0 ? `${strokeWidthPx}px ${pin.tagTextStrokeColor}` : "none", textAlign: pin.tagTextAlign, writingMode: pin.tagVerticalWriting ? "vertical-rl" : "horizontal-tb", letterSpacing: `${pin.tagLetterSpacing}em`, lineHeight: pin.tagLineHeight, padding: `${paddingYPx}px ${paddingXPx}px`, whiteSpace: "nowrap" }}>
